...
private:
  ...
  // For quantization all layers
  void DoQuantization();
  void EditNetDescriptionDFP(caffe::NetParameter* param, const int bw);
  void EditNetDescriptionPow2(caffe::NetParameter* param, const int bw);
  int GetLayerIdx(const string layer_name);
